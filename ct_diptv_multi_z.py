...
class PGGAN_multi_z(nn.Module):
    def __init__(self, gan_model_pre, gan_model_post, z_number,image_size,layer_c_number):
        super(PGGAN_multi_z, self).__init__()
        self.z_number = z_number
        self.z_dim = (1, image_size, image_size)

        self.pre_model = gan_model_pre
        self.post_model = gan_model_post
        self.init = True

        self.mask_size = (image_size, image_size)
        self.layer_c_number = layer_c_number

    def input_size(self):
        return [(self.z_number, *self.z_dim), (self.z_number, self.layer_c_number)]

    def init_value(self, batch_size):
        z_estimate = torch.randn(
            (batch_size, self.z_number, *self.z_dim)).cuda()  # our estimate, initialized randomly
    ...
    def forward(self, z):
        ...
        for j in range(self.z_number):
            feature_maps_list.append(
                self.pre_model(z_estimate[:, j, :]) * alpha_estimate[:, j, :].view(-1, 1, 1))
...
def run(f_name, specific_result_dir, noise_sigma, num_iter, rho, sigma_0, L):
    ...
    G1 = skip(1, layer_c_number,
              num_channels_down=[8, 16, 32],
              num_channels_up=[8, 16, 32],  # [16, 32, 64, 128, 128],
    ...
    G2 = skip(layer_c_number, 1,
              num_channels_down=[32, 64, 64],
              num_channels_up=[32, 64, 64],  # [16, 32, 64, 128, 128],
              num_channels_skip=[0,0,0],
              filter_size_up=3, filter_size_down=3, filter_skip_size=1,
              upsample_mode='bilinear',  # downsample_mode='avg',
              need1x1_up=False,
              ...
              act_fun='ELU').type(dtype)
    #%%
    multiG = PGGAN_multi_z(G1,G2, z_number,image_size,layer_c_number)
    z = torch.zeros((1,z_number, 1, image_size, image_size)).type(dtype).normal_()
    alpha = torch.zeros((1, z_number, multiG.layer_c_number)).type(dtype).normal_()

    x =(multiG([z,alpha])*0.5+0.5).clone().detach()
    ...
    opt_z = optim.AdamW(multiG.parameters(), lr=L, weight_decay=1e-4, amsgrad=True)

    sigma_0 = torch.tensor(sigma_0).type(dtype)
    Gz = (multiG([z,alpha])*0.5+0.5)
    ...
    for t in range(num_iter):
        ...
        loss_z.backward()
        nn.utils.clip_grad_norm_(multiG.parameters(), max_norm=2.0, norm_type=2)
        opt_z.step()

        # for dual var(lambda)
        with torch.no_grad():
            Gz = (multiG([z,alpha])*0.5+0.5).detach()
            x_Gz = x - Gz
        ...
        if (t + 1) % 1000 == 0:
            imsave(specific_result_dir + 'iter%d_PSNR_%.2f.png' % (t, psnr_gt), results[0,0].cpu().numpy(), cmap=cmap)
